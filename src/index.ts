...
function addTask(tasks: readonly Task[], newTask: Task): Task[] {
    return [...tasks, newTask];
}

async function getValidStatus(): Promise<Status> {
    const validStatuses: Status[] = [
        STATUS.PENDING,
        STATUS.IN_PROGRESS,
        STATUS.COMPLETED,
...
async function getValidDifficulty(): Promise<Difficulty> {
    const validDifficulties: Difficulty[] = [
        DIFFICULTY.EASY,
        DIFFICULTY.MEDIUM,
        DIFFICULTY.HARD
...
Ingresa uno: `)).toLowerCase();
        ...
        if (validDifficulties.includes(input as Difficulty)) {
